...
取込日はファイルの更新日時から決まるので、保存し直し忘れると前の日に入る。そういう場合と、大引け前に
保存して国内株式が前営業日の終値のままになっている場合は `[note]` で知らせる。

```
[note] 保有商品一覧…楽天証券[PC].html: 09/10 17:12 保存のファイルです。2026-09-10 分として取り込みました。今日の値にするには保存し直してください
[note] 口座管理｜SBI証券.html: 08:39 保存なので国内株式は前営業日の終値です。当日の終値を入れるなら 15:30 以降に保存し直してください
```
