...
- ログイン自動化はしない（認証情報を保持せず、規約上も通常のブラウザ利用の範囲）。ページの取得だけは Playwright で自動化できる（`portfolio fetch`）
...
証券会社サイトの対象ページを HTML として `imports/` 配下に置く（`imports/` は git 管理外）。SBI証券の5画面は **A. 自動取得**（`portfolio fetch`）で取れる。**楽天証券は当面 B. 手動保存のみ**（ページ URL にセッション ID が含まれ、メニュー遷移の自動化が未実装）。Playwright を入れたくない場合も B を使う。つまり日次の作業は「`portfolio fetch` 1回 + 楽天2画面を Ctrl+S」になる。

| 画面 | 保存先 | `fetch` キー | SBI証券 | 楽天証券 |
|---|---|---|---|---|
| 資産残高サマリ（現金含む） | `imports/assets/` | `sbi-assets` | My資産 → 資産残高（手動時は「詳細を表示する」を開いてから保存） | 保有商品一覧の上部から自動取得（別ページ不要） |
| 保有一覧（株式） | `imports/` | `sbi-foreign` | 口座管理 → 外国株式 → 保有銘柄 | 口座管理 → 保有商品一覧 → すべて |
| 保有一覧（国内株式・ETF + 投資信託） | `imports/funds/` | `sbi-account` | 口座管理 → 口座(円建) → 保有証券一覧（旧サイト） | （未対応） |
| 保有ファンド（投資信託のみ） | `imports/funds/` | `sbi-funds` | 投資信託 → 保有ファンド ※任意 | （未対応） |
| 注文照会 | `imports/orders/` | `sbi-orders` | 取引 → 外国株式 → 注文照会 | 米国株式取引 → 注文照会・訂正・取消 |

- SBI の「保有証券一覧」は国内株式と投資信託の両方を含むので、これ1枚で `holdings`（国内株式）と `funds` の両方に入る。「保有ファンド」ページは同じ投信データに **前日比と積立設定中フラグ** を足すだけなので、不要なら保存しなくてよい（両方取り込んだ場合は同じ行が上書きされ、後から取り込んだ方が残る。`fetch` はこの順で取り込む）
- SBI の保有証券一覧・保有ファンド画面には日付が表示されないため、保存ファイルの更新日時が `snapshot_date` になる。別の日に取り込む場合は `--date` で指定する

#### A. 自動取得: `portfolio fetch`（SBI証券）

Playwright がブラウザを開き、上の SBI 5画面を順に表示して DOM を保存し、そのまま取り込む。**ログインは手動**（ID・パスワード・デバイス認証コードはスクリプトに渡さない）。ログイン状態は永続プロファイル（既定 `tmp/pw-sbi`、git 管理外）に残るので、2回目以降はログインだけ、あるいはセッションが生きていれば操作なしで完走する。

```bash
uv sync --extra dev --extra fetch          # playwright を追加
uv run playwright install chromium         # 初回のみ（ブラウザ本体の取得）
uv run portfolio fetch --login-check       # 初回: ログイン + デバイス認証を済ませてプロファイルに保存

uv run portfolio fetch                     # 日次: SBI 5画面を保存 → DB に取込（ブラウザが開いたらログイン）
uv run portfolio fetch sbi-assets sbi-foreign   # 画面を指定
uv run portfolio fetch --no-import         # 保存だけ（後で portfolio import）
uv run portfolio fetch && uv run portfolio report --open   # 取込からレポート表示まで
```

- ログイン完了はページ内容（「ログアウト」リンクの出現）で自動検知する（最大5分）。1画面が取れなくても残りは続行し、終了コード 1 と警告で知らせる
- 保存先は `imports/<種別>/YYYYMMDD-<キー>.html`。画像や CSS は保存せず DOM だけ（`page.content()`）なので手動保存の半分以下のサイズ。同じ日に再実行すると上書きされる
- 対応画面は `src/portfolio/fetch.py` の `PAGES`。URL と「描画完了とみなす本文の文字列」を1エントリ足せば増やせる（SPA のページはこの目印が出るまで待ってから保存する）
- 別のスクリプトで作ったプロファイルを使い回すには `--profile <dir>` か環境変数 `PORTFOLIO_PW_PROFILE`。同じプロファイルを2つのブラウザで同時には開けない
- 共有PCではプロファイルを残さないこと
- 楽天証券は未対応。楽天の保有商品一覧・注文照会は引き続き B の手動保存で `imports/` に置き、`portfolio fetch` の後に `portfolio import` を全件で走らせる（または `fetch --no-import` → 楽天を保存 → `import`）

#### B. 手動保存: `Ctrl+S`

証券会社サイトにログインし、対象ページを開いて `Ctrl+S` →「ウェブページ、完全」で保存する（ファイル名は任意）。ページ種別は内容から自動判定するので置き場所は自由だが、整理のため上の表の保存先を推奨。楽天証券は現状この方法だけ。SBI証券も A が使えない環境ではこちらで同じ結果になる。
...
**保存は平日 21:00〜22:30 が最適。** 3つの市場のタイミングが同時に満たされる唯一の時間帯:

| | 15:30 | 21:00〜22:30 | 22:30以降 |
|---|---|---|---|
| 国内株式（東証 15:30 引け） | 当日終値 | 当日終値 | 当日終値 |
| 投資信託（基準価額は夕方公表） | 前日公表分 | 当日公表分 | 当日公表分 |
| 米国株式（22:30 寄り付き、冬時間 23:30） | 前夜の終値 | 前夜の終値 | **場中の値** |

取込日はファイルの更新日時から決まるので、保存し直し忘れると前の日に入る。それと、上の時間帯を外れて
その画面の値が当日分でない場合は `[note]` で知らせる（該当する資産を含むページにだけ出る）。

```
[note] 保有商品一覧…楽天証券[PC].html: 09/10 17:12 保存のファイルです。2026-09-10 分として取り込みました。今日の値にするには保存し直してください
[note] 口座管理｜SBI証券.html: 08:49 保存なので国内株式は前営業日の終値です（東証は 15:30 引け）。当日の終値を入れるなら 平日 21:00〜22:30 に保存し直してください
[note] 口座管理｜SBI証券.html: 08:49 保存なので投資信託は当日公表分の基準価額ではない可能性があります。平日 21:00〜22:30 に保存し直すと揃います
```
